class Solution {
    ...
    public int[][] kClosest2(int[][] raw, int k) {
    ...
    public int[][] kClosest(int[][] raw, int k) {        
        PriorityQueue<Point> pq = new PriorityQueue<>(Comparator.<Point, Integer>comparing(p -> p.d));
        for (int i = 0; i < raw.length; i++) {
            pq.offer(new Point(raw[i][0], raw[i][1]));
        }
        
        int[][] results = new int[k][2];
        for (int i = 0; i < k; i++) {
            results[i] = pq.poll().asArray();
        }
